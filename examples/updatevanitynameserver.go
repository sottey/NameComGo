...
func UpdateVanityNameserverExample() {
    fmt.Println("Executing UpdateVanityNameserver() example...\n")
    ...
    // Make the call
    nameResponse := namecomgo.UpdateVanityNameserver(c, vanityServer)
    ...
    switch nameResponse.StatusCode {
    	case 200:
            ...
            namecomgo.Check("Error in UpdateVanityNameserver(): ", jsonErr)
            
            // Display results
            if ret.DomainName != "" {
                fmt.Printf("\nDomain: %v\nHostName: %v\nIP Count: %v\n\n", ret.DomainName, ret.Hostname, len(ret.IPs))
            } else {
                fmt.Printf("Empty UpdateVanityNameserver response!")
            }
    	case 404:
